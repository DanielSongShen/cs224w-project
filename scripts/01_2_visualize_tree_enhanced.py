...
def visualize_tree(tree_dict: Dict[str, Any], item_data: Dict[str, Any],
    ...
    # Create figure with two subplots: one for graph, one for text
    ...
    # === Text panel with reasoning sketch ===
    ax_text.axis('off')

    # Prepare text content showing reasoning sketch
    text_lines = []

    # Add prompt if available
    if 'full_prompt' in item_data:
        prompt_text = item_data['full_prompt'].replace('\\', '/')
    ...
    text_lines.append("=== REASONING SKETCH ===")
    text_lines.append("")

    if 'reasoning_sketch' in item_data:
        reasoning_sketch = item_data['reasoning_sketch']
        
        # Extract content from <reasoning_process> tags if present
        if '<reasoning_process>' in reasoning_sketch:
            start_idx = reasoning_sketch.find('<reasoning_process>') + len('<reasoning_process>')
            end_idx = reasoning_sketch.find('</reasoning_process>')
            if end_idx != -1:
                reasoning_sketch = reasoning_sketch[start_idx:end_idx].strip()
        
        # Sanitize LaTeX characters
        reasoning_sketch = reasoning_sketch.replace('\\', '/')
        
        # Split by lines and wrap long lines
        sketch_lines = reasoning_sketch.split('\n')
        max_line_length = 60
        ...
        text_lines.append("No reasoning sketch available")

    # Display text in the text panel
    full_text = '\n'.join(text_lines)
    ax_text.text(
        0.05, 0.95, full_text,
...
def find_next_available_tree_name(output_dir: Path) -> str:
    """
    Find the next available tree_n.png filename by incrementing counter.
    
    Args:
        output_dir: Directory where trees are saved
    
    Returns:
        Path to next available tree file (e.g., "tree_0.png", "tree_1.png", etc.)
    """
    counter = 0
    while True:
        filename = f"tree_{counter}.png"
...
def visualize_all_trees(input_path: str, output_dir: str, max_trees: int = None,
                        show_thoughts: bool = False, skip_existing: bool = True):
    """
    ...
    Args:
        ...
        skip_existing: If True, use counter-based naming (tree_0.png, tree_1.png, ...)
                      that finds next available number to avoid overwriting.
                      If False, use tag-based naming (can overwrite existing files).
    """
    ...
    for i, item in enumerate(data):
        ...
        # Generate output filename based on skip_existing setting
        if skip_existing:
            # Use counter-based naming that finds next available number
            output_path = find_next_available_tree_name(output_dir)
        else:
            # Use tag-based naming (can overwrite)
            tag = item.get('tag', f'item_{i}')
            safe_tag = tag.replace('/', '_').replace('\\', '_')
            output_path = str(output_dir / f"{safe_tag}.png")
        
        print(f"\nVisualizing tree {i+1}/{len(data)}: {item.get('tag', 'unknown')}")
        ...
        try:
            ...
            print(f"Error visualizing tree {item.get('tag', 'unknown')}: {str(e)[:100]}")
...
def main():
    ...
    parser.add_argument(
        '--use_tags',
        action='store_true',
        help='Use tag-based naming instead of counter-based naming (can overwrite existing files)'
    )
    ...
    if not os.path.exists(args.input_path):
        ...
        print("  python scripts/01_1_visualize_tree.py --use_tags  # Use original tag-based naming")
        return 1
    ...
    visualize_all_trees(
        ...
        skip_existing=not args.use_tags  # Counter-based by default, tag-based if --use_tags
    )
